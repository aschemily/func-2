...
// CODE HERE
const totalPopulation = numbers.reduce((acc, cv )=>{
  return acc + cv
})
...
let strongMonster = monstersInYourPocket.filter(monster => {
  return monster.CP > 200
})

//console.log(strongMonster)
...
// CODE HERE
//let totalArray = []
let totalPrice = orders.map(el => {
  //console.log('what is', el.price, el.tax)
...
//    console.log(acc.price + cv.price)
//    return acc.price + cv.price
// })
let total = 0
let arrayPrice = []
let bobsTotal = purchases.map(el =>{
 // console.log('el is', el)
  if(el.owner === 'Bob'){
    //console.log('el is', el.price)
    total += el.price
